...
from MI_estimators import CLUB, CLUBv2
...
def train_mi_estimator(mi_estimator, outputs, inputs=None):
...
def eval_mi_estimator(mi_estimator, outputs, inputs=None):
...
def train(args, train_dataset, model, tokenizer, mi_estimator=None):
    """ Train the model """
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if mi_estimator:
                upper_bound = train_mi_estimator(mi_estimator, outputs, inputs) / args.adv_steps
                loss += upper_bound
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:
                        results = evaluate(args, model, tokenizer, mi_estimator=mi_estimator)
                    ...
                    if mi_estimator:
                        tb_writer.add_scalar("loss", (upperbound_loss - logging_up_loss) / args.logging_steps, global_step)
...
def evaluate(args, model, tokenizer, prefix="", adv=True, mi_estimator=None):
    ...
    result = evalute_dataset(args, model, datasets, tokenizer, prefix, mi_estimator)
    results = [result]
    ...
    if adv:
        ...
        for i, dataset in enumerate(datasets):
            prefix = prefixs[i]
            results.append(evalute_dataset(args, model, dataset, tokenizer, prefix, mi_estimator))
    return results


def evalute_dataset(args, model, datasets, tokenizer, prefix="", mi_estimator=None):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            outputs = model(**inputs)
            if mi_estimator:
                mi_info += eval_mi_estimator(mi_estimator, outputs, inputs),
        ...
        for i, feature_index in enumerate(feature_indices):
            ...
            if len(output) >= 5:
                ...
                # if mi_estimator:
                start_logits, end_logits, hidden = output
                # else:
...
def main():
    ...
    if args.version >= 0:
        if args.version == 0:
            mi_estimator = CLUB(config.hidden_size * args.max_seq_length, config.hidden_size,
                                beta=args.beta).to(args.device)
        elif args.version == 1:
            mi_estimator = CLUB(config.hidden_size, config.hidden_size, beta=args.beta).to(args.device)
            # mi_estimator = CLUB(config.hidden_size, config.hidden_size,  beta=args.beta)
        elif args.version == 2 or args.version == 3:
            mi_estimator = CLUBv2(config.hidden_size, config.hidden_size, beta=args.beta).to(args.device)

        mi_estimator.version = args.version
    else:
        mi_estimator = None
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset, model, tokenizer, mi_estimator)

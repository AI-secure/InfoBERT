...
class Trainer:
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if self.mi_estimator:
                    ...
                    tr_loss += self._adv_training_step(model, inputs, optimizer)
    ...
    def _train_mi_estimator(self, outputs, inputs=None):
        ...
        if self.mi_estimator.version == 0:
            ...
            return self.mi_estimator.update(embedding_layer, embeddings)
    ...
    def _eval_mi_estimator(self, outputs, inputs=None):
        ...
        if self.mi_estimator.version == 0:
            ...
            for i, length in enumerate(lengths):
                embeddings.append(embedding_layer[i, :length])
            embeddings = torch.cat(embeddings)  # [-1, 768]
            return self.mi_estimator.mi_est(embedding_layer, embeddings)
    ...
    def _adv_training_step(
        ...
        tr_loss, upperbound_loss = 0.0, 0.0
        ...
        for astep in range(self.args.adv_steps):
            ...
            if self.mi_estimator:
                upper_bound = self._train_mi_estimator(outputs, inputs) / self.args.adv_steps
                loss += upper_bound
        ...
        if self.mi_estimator:
            loss_dict = {"task_loss": tr_loss, "upper_bound": upperbound_loss}
